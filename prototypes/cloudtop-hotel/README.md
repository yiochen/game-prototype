...
Guest Parade costs 7 coins. Start the next typed purchase with a guest different from the hotel's current top floor to earn +1, +2, +3… bonus rooms. On an empty hotel any guest qualifies. All 24 Mosaic variants compare their first printed guest with the top floor and advance once when those types differ, regardless of internal repeats. Matching the top floor ends the parade before a bonus. The bonus batch is one uniformly random type different from the purchased guest (or final printed Mosaic guest), appended after normal output. These bonus rooms become the top floor for the next comparison. For example, Cat → Frog bonus → Cat continues. Untyped powers pause the bonus; any rooms they create still change the top floor used next. Typed upgrades and Type Lock count, and Room Choice evaluates the chosen guest. Only one of Guest Parade and Neighborhood Streak can be active.
