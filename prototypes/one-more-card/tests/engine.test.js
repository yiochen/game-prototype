...
test('Recall replaces Polish, counts segments regardless of length, and preserves existing links',()=>{
 ...
 buy(s,'growth','recall','sun');assert.equal(chain(s),8);
 assert.deepEqual(s.links.slice(0,before.length),before);
 assert.deepEqual(segments(s).map(r=>r.length),[4,3,1]);
 buy(s,'growth','recall','sun');assert.equal(s.history.at(-1).links,2);assert.equal(chain(s),10);
 buy(s,'growth','overgrow');assert.equal(chain(s),12);
 buy(s,'growth','recall','sun');assert.equal(s.history.at(-1).links,2);assert.equal(chain(s),14);
...
test('splitting six Sun links increases Recall; purchase boundaries do not',()=>{
 ...
 assert.equal(segments(long).length,1);assert.equal(recallLinks(long,'sun'),1);assert.equal(recallLinks(split,'sun'),2);
 assert.equal(preview(split,offer(split,'growth','recall','sun')).headline,'+2');
});
test('Recall snapshots once, appends its own suit, and only future purchases reuse those links',()=>{
 ...
 const third=buy(s,'growth','recall','sun');assert.equal(third.links,2);assert.equal(segments(s,'sun').length,2);
});
test('Overgrow appends the longest run’s suit, snapshots its length, and caps its bonus',()=>{
 ...
 for(const amount of [4,4,4,6,8]){
  ...
  assert.deepEqual(s.links.slice(0,before.length),before);assert.ok(s.links.slice(before.length).every(l=>l.suit==='sun'));
 }
 assert.deepEqual(segments(s).map(r=>[r.suit,r.length]),[['sun',9],['moon',1],['sun',26]]);
 const tie=createGame('tie');buy(tie,'base','single','moon');buy(tie,'base','single','sun');buy(tie,'growth','overgrow');
 assert.deepEqual(segments(tie).map(r=>[r.suit,r.length]),[['moon',1],['sun',1],['moon',1]]);
});
test('upgrade levels replace each other, disappear at max, and affect only future purchases',()=>{
 ...
 const recalled=buy(s,'growth','recall','sun');assert.equal(recalled.links,1);
});
test('Stabilizer uses explicit odds in one draw, with Reactor applied afterward',()=>{
 ...
 const r=buy(s,'growth','recall','moon');assert.equal(r.links,1);assert.ok(s.history.every(c=>!('nodes' in c)));
});
...
test('Choice 1 validates selection before spending and stacks the chosen Reactor, Assembler, Rebate and Foundation',()=>{
 const s=createGame('choice');buy(s,'reactor','suit','moon');buy(s,'reactor','assembler');buy(s,'wealth','rebate');buy(s,'strategy','foundation');
 ...
 assert.equal(preview(s,c,'moon').headline,'+4');assert.equal(preview(s,c,'sun').headline,'+3');assert.deepEqual(s,before);
 assert.equal(pick(s,0,'moon'),true);assert.equal(s.history.at(-1).suit,'moon');assert.equal(s.history.at(-1).links,4);assert.equal(s.cash,before.cash-c.price+2);assert.equal(s.foundation.bonus,1);
 assert.deepEqual(s.links.map(l=>l.suit),Array(4).fill('moon'));
});
...
test('Foundation advances on matching suited purchases, pauses on suitless cards and ends on switching',()=>{
 ...
 for(const [family,type,amount] of [['base','single',2],['growth','recall',3],['reactor','suit',3]]){
  const c=offer(s,family,type,'sun');assert.equal(preview(s,c).foundationBonus,s.foundation.bonus+1);
 ...
 const c=offer(s,'base','single','moon');assert.equal(preview(s,c).endsFoundation,true);assert.equal(preview(s,c).headline,'+1');
 pick(s,0);advanceDeal(s);assert.equal(s.foundation,null);assert.equal(s.history.at(-1).links,1);
 assert.ok(eligibleCards(s).some(c=>c.type==='foundation'));buy(s,'strategy','foundation');buy(s,'base','single','moon');assert.equal(s.foundation.bonus,1);
});
...
test('Attunement locks exactly the next shops including Choice, pauses Mosaic, and does not refresh',()=>{
 const s=createGame('attunement');buy(s,'strategy','foundation');buy(s,'strategy','attunement','moon');
 assert.deepEqual(s.attunement,{suit:'moon',remaining:3});assert.equal(s.foundation.bonus,1);
 const prize=s.prizeRng;
 for(let remaining=3;remaining>0;remaining--){
  assert.equal(s.attunement.remaining,remaining);
  for(const c of [...s.offer,...eligibleCards(s)]){assert.ok(!c.suit||c.suit==='moon');assert.ok(!c.sequence);assert.notEqual(c.type,'attunement');}
  assert.deepEqual(choiceSuits(s),['moon']);
  if(remaining===3){
   offer(s,'base','choice');const before=structuredClone(s);assert.equal(pick(s,0,'sun'),false);assert.deepEqual(s,before);
   assert.equal(pick(s,0,'moon'),true);advanceDeal(s);
  } else if(remaining===2) buy(s,'wealth','rebate');
  else buy(s,'base','single','moon');
 }
 assert.equal(s.attunement,null);assert.equal(s.foundation.bonus,3);assert.equal(s.prizeRng,prize);
 assert.equal(choiceSuits(s).length,3);assert.ok(eligibleCards(s).some(c=>c.type==='mosaic'));assert.ok(eligibleCards(s).some(c=>c.suit==='sun'));assert.ok(eligibleCards(s).some(c=>c.type==='attunement'));
});

test('Attunement preserves affordable fallback and final-purchase settlement',()=>{
 for(let seed=0;seed<30;seed++){
  const s=createGame(seed);buy(s,'strategy','attunement','wave');s.cash=7;
  offer(s,'base','choice');pick(s,0,'wave');advanceDeal(s);
  assert.ok(s.offer.some(c=>c.price<=3));assert.ok(s.offer.every(c=>!c.suit||c.suit==='wave'));assert.equal(new Set(s.offer.map(c=>c.id)).size,s.offer.length);
  offer(s,'base','single','wave');pick(s,0);assert.equal(finished(s),false);advanceDeal(s);assert.equal(finished(s),true);assert.equal(chain(s),2);
 }
});
